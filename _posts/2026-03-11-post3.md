...
We also perform the module installation (which involves moving the modules to the correct location by mounting the VM disk, copying the modules there, and then unmounting). The tutorial unmount step is using a var that was not created. The corrected version is bellow:
