...
```bash
git commit --signoff --trailer='Co-developed-by: Pietro Di Consolo Gregorio <pietro.gregorio@usp.br>' \ --trailer='Signed-off-by: Pietro Di Consolo Gregorio <pietro.gregorio@usp.br>' -m "iio: \
HID: Add helper method hid_sensor_adjust_channel_bit_mask()" -m "Add helper method to deduplicate code in HID sensors."
```
